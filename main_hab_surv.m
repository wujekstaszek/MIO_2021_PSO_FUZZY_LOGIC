...
mf_output_classes_num = 2;          % number of classes in output functions
...
fis.name = "Haberman's survival classification problem fuzzy system";           % FIS name

% FL INPUTS
input_names = {'patient age'; 'operation year'; 'axillary nodes'};              % input variables' names
...
% FL OUTPUTS
output_names = ['Survival status'];                                             % outputs' names
mf_output_names = {'Survived','Died'};                                          % names of output variables' membership functions
...
function result = test_function(vars)
    ...
    result = floor(evalfis(fis,test(:,1:inputs_num))*(4/3)+1);      % projecting results to classes
...
function procentage_result = updateVariables(vars)
    ...
    global results
    results = floor(evalfis(fis,learn(:,1:inputs_num))*(4/3)+1);    % projecting results to classes
